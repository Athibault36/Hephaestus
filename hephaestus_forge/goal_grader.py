...
def _displacement_command_succeeded(memory: Optional[list[dict[str, Any]]]) -> bool:
    for step in memory or []:
        if not step.get("ok"):
            continue
        if step.get("kind") in ("apply_move", "move_actor"):
            return True
        if step.get("command") in ("world.apply_move_input", "world.move_actor"):
            return True
    return False


def _displacement_goal(goal_l: str) -> bool:
    return any(
        phrase in goal_l
        for phrase in ("displacement", "moved forward", "walk forward", "move forward", "walked")
    )
...
def grade_goal(goal: str, snapshot: Any, memory: Optional[list[dict[str, Any]]] = None) -> GradeResult:
    ...
    if _displacement_goal(goal_l) and not _displacement_command_succeeded(memory):
        pawn_state = getattr(snapshot, "pawn_state", None) or {}
        pawn_speed = float(pawn_state.get("speed") or 0.0) if isinstance(pawn_state, dict) else 0.0
        if pawn_speed < 10.0:
            missing.append("displacement not observed")
